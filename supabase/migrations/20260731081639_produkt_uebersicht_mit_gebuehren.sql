-- Produkt-Übersicht um die ECHTEN Amazon-Gebühren je ASIN erweitern.
--
-- Zuordnung: finance_gebuehren führt SKU -> über orders_history/fba_bestand auf
-- die ASIN gemappt. Gebühren liegen MONATLICH vor, der Zeitraum ist frei
-- wählbar -> anteilig nach Tagesüberlappung. Das ist eine Näherung (Gebühren
-- fallen im Monat nicht gleichmäßig an) und wird als `gebuehren_anteilig`
-- ausgewiesen, wenn ein Monat nur teilweise im Zeitraum liegt.
-- Gebühren ohne SKU (Lager-/Service-Fees) sind hier NICHT enthalten — sie sind
-- nicht produktscharf und würden den Produkt-Nettogewinn verfälschen.
...
as $function$
  ...
  -- SKU -> ASIN. Bestellungen zuerst, Lagerbericht als Ergänzung.
  sku_map as (
  ...
  -- Monatsgebühren je ASIN, anteilig auf den Zeitraum.
  geb as (
    select m.asin,
    ...
    join lateral (
      select s.asin as asin,
             -- Ueberlappung des Monats mit [p_von, p_bis] in Tagen / Monatstage
             greatest(0,
