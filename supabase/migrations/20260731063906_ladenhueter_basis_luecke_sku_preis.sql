-- Ladenhüter-Radar schärfen: „keine Nachfrage" von „keine Ware" unterscheiden.
-- Neu je ASIN: längste Versorgungslücke (Null-Strecke), SKU-Wechsel (neue Charge)
-- und Preiswechsel. Ohne diese Signale wurde ein Stockout als Ladenhüter gewertet.
...
as $function$
  with basis as (
    select o.asin,
           ...
           -- Ø Stückpreis vorher/jetzt (nur wo Preis bekannt)
  ...
  -- Verkaufstage der letzten 120 Tage -> längste Strecke OHNE Verkauf.
  vtage as (
  ...
  luecken as (
    select asin, greatest((d - lag(d) over (partition by asin order by d)) - 1, 0) as luecke
    from vtage
  ),
  maxluecke as (
    select asin, coalesce(max(luecke), 0)::int as max_luecke_tage from luecken group by asin
  ),
  -- SKU-Wechsel: verkauft sich jetzt unter einer SKU, die es im Vorquartal nicht gab?
  skuneu as (
    select b.asin,
           exists (
