-- Backfill um den Ledger-SUMMARY-Report erweitern (Bestandshistorie).
-- Gleiche Chunk-Groesse wie der Ledger-Detail-Report (60 Tage, Amazon-Limit) und
-- dasselbe 18-Monats-Fenster. Unveraendert uebernommen: alle bisherigen Bloecke.
...
begin
  ...
  -- Bestandshistorie: taeglicher Lagerstand (Ledger-Summary, 60-Tage-Chunks) über 18 Monate.
  with fenster as (select gs as k from generate_series(0, ceil(540/60.0)::int - 1) gs),
       ins as (
         insert into public.backfill_jobs (tenant_id, report_type, von, bis, status)
         select p_tenant, 'GET_LEDGER_SUMMARY_VIEW_DATA',
                (current_date - ((f.k+1)*60))::date, (current_date - (f.k*60))::date, 'pending'
         from fenster f
         where not exists (select 1 from public.backfill_jobs b
           where b.tenant_id=p_tenant and b.report_type='GET_LEDGER_SUMMARY_VIEW_DATA'
             and b.von=(current_date-((f.k+1)*60))::date and b.bis=(current_date-(f.k*60))::date)
         returning 1)
  select count(*) into v_add from ins;
  v_ins := v_ins + v_add;
...
-- Taeglicher Lauf: 45-Tage-Fenster (repariert auch nachtraeglich korrigierte Tage).
insert into internal.scheduler_reports (report_type, days, aktiv)
values ('GET_LEDGER_SUMMARY_VIEW_DATA', 45, true)
on conflict (report_type) do update set days = excluded.days, aktiv = true;;
