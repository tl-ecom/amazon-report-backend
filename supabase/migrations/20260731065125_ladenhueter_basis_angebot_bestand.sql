-- Ladenhüter-Radar um den ANGEBOTSSTATUS erweitern. Viele „tote" ASINs sind
-- schlicht ausgelistet — dann ist „Relaunch oder auslisten" sinnlos, weil das
-- Angebot längst weg ist. Bestandsmenge gibt es nur für Merchant-Artikel;
-- FBA-Mengen liefert dieser Report nicht (bleibt NULL = unbekannt, nie 0).
...
returns table(
  ...
  max_luecke_tage integer,
  neue_sku boolean,
  preis_alt_cents integer,
  preis_neu_cents integer,
  hat_angebot boolean,
  angebot_status text,
  bestand integer,
  ist_fba boolean
...
as $function$
  ...
  luecken as (
    select asin, greatest((d - lag(d) over (partition by asin order by d)) - 1, 0) as luecke from vtage
  ),
  maxluecke as (
    select asin, coalesce(max(luecke), 0)::int as max_luecke_tage from luecken group by asin
  ),
  skuneu as (
    select b.asin,
           exists (
             ...
             where n.tenant_id = p_tenant and n.asin = b.asin and n.quantity > 0
               ...
               and not exists (
                 select 1 from public.orders_history a
  ...
  -- Jüngster Angebots-Snapshot je ASIN: aktiv? welcher Kanal? Menge (nur Merchant).
  angebot as (
    select distinct on (s.asin)
           s.asin, s.status, s.quantity, s.is_fba
    from public.asin_snapshots s
  ...
  select b.asin, b.lifetime_units, b.lifetime_umsatz_cents,
         ...
         (g.asin is not null) as hat_angebot,
         g.status as angebot_status,
         g.quantity as bestand,
         coalesce(g.is_fba, false) as ist_fba
  from basis b
