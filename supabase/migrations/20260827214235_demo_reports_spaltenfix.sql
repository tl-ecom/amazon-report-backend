-- Korrektur zur vorigen Migration: Die Angebotsliste griff auf
-- fba_bestand.menge zu — die Spalte heisst verkaufsfaehig. Die erste Fassung
-- war damit nicht lauffaehig; dieser Nachtrag stellt den tatsaechlich geprueften
-- Stand her (fuenf Reports erzeugt, Konsistenzpruefung nach Datum vs. nach ASIN
-- in allen sechs Kennzahlen deckungsgleich).
...
begin
  ...
  select jsonb_build_object(
    ...
    'rows', coalesce(jsonb_agg(jsonb_build_object(
        ...
        'quantity', coalesce(b.verkaufsfaehig, 0)::text,
