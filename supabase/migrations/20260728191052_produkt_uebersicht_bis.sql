-- Obergrenze p_bis ergänzen (Default = heute) für frei wählbare Zeiträume.
drop function if exists public.produkt_uebersicht(uuid, date);
create or replace function public.produkt_uebersicht(p_tenant uuid, p_von date, p_bis date default current_date)
...
revoke execute on function public.produkt_uebersicht(uuid,date,date) from anon, authenticated, public;
