...
export async function ladeEinstellungen(supabase: any, tenant_id: string): Promise<unknown> {
  const { data, error } = await supabase.from("tenant_einstellungen")
    .select("ziel_acos_prozent, kosten_abschlag_prozent").eq("tenant_id", tenant_id).maybeSingle();
...
export async function setzeEinstellungen(
  supabase: any, tenant_id: string, args: { ziel_acos_prozent?: unknown; kosten_abschlag_prozent?: unknown },
): Promise<{ ok: true }> {
  ...
  const { error } = await supabase.from("tenant_einstellungen").upsert({
    tenant_id, ziel_acos_prozent: ziel, kosten_abschlag_prozent: abschlag, updated_at: new Date().toISOString(),
