...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Sponsored Brands: Tagesbudget setzen {campaignId, budget}
    if (action === "sb_budget_setzen") {
      if (!bestaetigt()) return json({ error: "bestaetigung=true fehlt. Es wurde NICHTS geschrieben." }, 400);
      ...
      if (!r.ok || !r.daten.length) return json({ error: "SB-Kampagne nicht gefunden", detail: r.ok ? null : r.detail }, 404);
      const c = r.daten[0];
      const vorher = typeof c.budget === "number" ? c.budget : null;
      const budgetType = c.budgetType ?? "DAILY";
      if (vorher !== null && Math.abs(vorher - budget) < 0.005) {
        await spur([{ aktion: "sb_budget_setzen", objekt_art: "sb_campaign", objekt_id: cid, campaign_id: cid, vorher: { budget: vorher, budgetType }, nachher: { budget, budgetType }, ergebnis: "uebersprungen", detail: "schon so" }]);
      ...
      const put = await ads.put("/sb/v4/campaigns", CT.sbCampaign, "campaigns", "campaignId", [{ campaignId: cid, budget, budgetType }]);
      const e = put.get(cid) ?? { ok: false, detail: "keine Antwort" };
      const logErr = await spur([{ aktion: "sb_budget_setzen", objekt_art: "sb_campaign", objekt_id: cid, campaign_id: cid, vorher: { budget: vorher, budgetType }, nachher: { budget, budgetType }, ergebnis: e.ok ? "ok" : "fehler", detail: e.ok ? null : JSON.stringify(e.detail).slice(0, 1000) }]);
